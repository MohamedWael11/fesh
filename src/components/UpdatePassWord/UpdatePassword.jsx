...
import BarLoader from 'react-spinners/esm/BarLoader';
...
function UpdatePassword() {
    ...
    async function callUpdatePassword(values) {
        try {
            setiscalling(true)
            setapierror(null)
    ...
    return (
        <div>
            <form onSubmit={formik.handleSubmit} className="w-1/2 mx-auto my-6">
                <h1 className="mb-6 text-3xl uppercase">Update Password:</h1>
                ...
                <div className="relative z-0 w-full mt-2 mb-5 group">
                    <input onBlur={formik.handleBlur} type="password" name="newPassword" value={formik.values.newPassword} onChange={formik.handleChange} id="floating_password" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-main focus:outline-none focus:ring-0 focus:border-main peer" placeholder=" " required />
                    <label htmlFor="floating_password" className="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-main peer-focus:dark:text-main peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">newPassword:</label>

                    {formik.errors.newPassword && formik.touched.newPassword ? <div className="p-2 mt-2 mb-4 text-sm text-red-800 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400" role="alert">
                ...
                </div> : <button type="submit" className="text-white ms-auto block hover:opacity-80 bg-main hover:bg-main focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-main dark:hover:bg-main dark:focus:ring-main">Update</button>}
            </form>
        </div>
    )
}
